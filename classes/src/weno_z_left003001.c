...
void
weights_z_left003001 (const double *restrict sigma, int n, int ssi, int ssr,
  ...
  for (i = 3; i < n - 3; i++)
    {
      ...
      omega0 = (+0.1) *( 1+ (tau /(sigma0 + 1.0e-6)) * (tau /(sigma0 + 1.0e-6)) );
      acc = acc + omega0;

      omega1 = (+0.6) *( 1+ (tau /(sigma1 + 1.0e-6)) * (tau /(sigma1 + 1.0e-6)) );
      acc = acc + omega1;
      
      omega2 = (+0.3) *( 1+ (tau /(sigma2 + 1.0e-6)) * (tau /(sigma2 + 1.0e-6)) );
      acc = acc + omega2;
...
PyObject *
py_weights_z_left003001 (PyObject * self, PyObject * args)
{
  double *sigma, *omega;
  ...
  weights_z_left003001 (sigma, n, ssi, ssr, omega, wsi, wsl, wsr);

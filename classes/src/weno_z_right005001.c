...
void
weights_z_right005001 (const double *restrict sigma, int n, int ssi, int ssr,
  ...
  for (i = 5; i < n - 5; i++)
    {
      ...
      acc = 0.0;
      omega0 = (+0.0396825396825397)*( 1+ (tau /(sigma0 + 1.0e-6)) * (tau /(sigma0 + 1.0e-6)) );
      acc = acc + omega0;
      omega1 = (+0.317460317460317)*( 1+ (tau /(sigma1 + 1.0e-6)) * (tau /(sigma1 + 1.0e-6)) );
      acc = acc + omega1;
      omega2 = (+0.476190476190476)*( 1+ (tau /(sigma2 + 1.0e-6)) * (tau /(sigma2 + 1.0e-6)) );
      acc = acc + omega2;
      omega3 = (+0.158730158730159)*( 1+ (tau /(sigma3 + 1.0e-6)) * (tau /(sigma3 + 1.0e-6)) );
      acc = acc + omega3;
      omega4 = (+0.00793650793650794)*( 1+ (tau /(sigma4 + 1.0e-6)) * (tau /(sigma4 + 1.0e-6)) );
      acc = acc + omega4;
...
PyObject *
py_weights_z_right005001 (PyObject * self, PyObject * args)
{
  double *sigma, *omega;
  ...
  weights_z_right005001 (sigma, n, ssi, ssr, omega, wsi, wsl, wsr);

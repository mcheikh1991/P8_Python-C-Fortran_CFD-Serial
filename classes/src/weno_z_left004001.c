...
void
weights_z_left004001 (const double *restrict sigma, int n, int ssi, int ssr,
  ...
  for (i = 4; i < n - 4; i++)
    {
      ...
      acc = 0.0;
      omega0 = (+0.0285714285714286)*( 1+ (tau /(sigma0 + 1.0e-6)) * (tau /(sigma0 + 1.0e-6)) );
      acc = acc + omega0;
      omega1 = (+0.342857142857143) *( 1+ (tau /(sigma1 + 1.0e-6)) * (tau /(sigma1 + 1.0e-6)) );
      acc = acc + omega1;
      omega2 = (+0.514285714285714) *( 1+ (tau /(sigma2 + 1.0e-6)) * (tau /(sigma2 + 1.0e-6)) );
      acc = acc + omega2;
      omega3 = (+0.114285714285714) *( 1+ (tau /(sigma3 + 1.0e-6)) * (tau /(sigma3 + 1.0e-6)) );
      acc = acc + omega3;
      omega0 = (omega0) / (acc);
      omega1 = (omega1) / (acc);
      omega2 = (omega2) / (acc);
...
PyObject *
py_weights_z_left004001 (PyObject * self, PyObject * args)
{
  double *sigma, *omega;
  ...
  weights_z_left004001 (sigma, n, ssi, ssr, omega, wsi, wsl, wsr);

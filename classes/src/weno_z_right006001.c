...
void
weights_z_right006001 (const double *restrict sigma, int n, int ssi, int ssr,
  ...
  for (i = 6; i < n - 6; i++)
    {
      ...
      acc = 0.0;
      omega0 = (+0.012987012987013)*( 1+ (tau /(sigma0 + 1.0e-6)) * (tau /(sigma0 + 1.0e-6)) );
      acc = acc + omega0;
      omega1 = (+0.162337662337662)*( 1+ (tau /(sigma1 + 1.0e-6)) * (tau /(sigma1 + 1.0e-6)) );
      acc = acc + omega1;
      omega2 = (+0.432900432900433)*( 1+ (tau /(sigma2 + 1.0e-6)) * (tau /(sigma2 + 1.0e-6)) );
      acc = acc + omega2;
      omega3 = (+0.324675324675325)*( 1+ (tau /(sigma3 + 1.0e-6)) * (tau /(sigma3 + 1.0e-6)) );
      acc = acc + omega3;
      omega4 = (+0.0649350649350649)*( 1+ (tau /(sigma4 + 1.0e-6)) * (tau /(sigma4 + 1.0e-6)) );
      acc = acc + omega4;
      omega5 = (+0.00216450216450216)*( 1+ (tau /(sigma5 + 1.0e-6)) * (tau /(sigma5 + 1.0e-6)) );
      acc = acc + omega5;
...
PyObject *
py_weights_z_right006001 (PyObject * self, PyObject * args)
{
  double *sigma, *omega;
  ...
  weights_z_right006001 (sigma, n, ssi, ssr, omega, wsi, wsl, wsr);

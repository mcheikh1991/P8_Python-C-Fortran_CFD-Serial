...
!* version if you wish.
!*
!* Ver.5 (11-13-2013): Godunov flux corrected. (Thanks to François Fraysse!)
...
!*****************************************************************************
 ...
 implicit none
 !real :: uL(3), uR(3)!  Input (conservative variables rho*[1, v, E])
 real :: uL(5), uR(5)! Input (Mixed Variables [rho, rho*v, e, rpsi, a] i.e. rpsi=p/rho)
 real :: KNP(3)      ! Output (numerical flux across L and R states)
!Local constants
 real :: gamma                        ! Ratio of specific heat.
 real :: zero, quarter, half, one, two
!Local variables
 real :: gm1, vstar
 real :: rho_neg, rho_pos, rhoU_neg, rhoU_pos ! Conservative variables.
 real :: e_neg, e_pos, U_neg, U_pos, p_neg, p_pos ! Primitive variables.
 real :: cSf_neg, cSf_pos             ! Speeds of sound
 real :: rPsi_neg, rPsi_pos
 real :: ap, am, phiv_neg ,phiv_pos
 real :: a_pos , a_neg , amaxSf ,aSf
 real :: aphiv_pos, aphiv_neg
...
! Continuity Flux
  ...
  ap        = max(max(phiv_pos + cSf_pos, phiv_neg + cSf_neg),zero)
  am        = min(min(phiv_pos - cSf_pos, phiv_neg - cSf_neg),zero)

  a_pos     = ap/(ap - am)

  amaxSf    = max(abs(am),abs(ap))
  ...
  amaxSf = max(abs(aphiv_pos), abs(aphiv_neg))

  KNP(1)   =  aphiv_pos*rho_pos + aphiv_neg*rho_neg   ! Coninuity Flux

  KNP(2)   = (aphiv_pos*rhoU_pos + aphiv_neg*rhoU_neg) &
          + (a_pos*p_pos + a_neg*p_neg) ! Linear Momentum Flux

  KNP(3) = aphiv_pos*(rho_pos*(e_pos + 0.5*(U_pos)**2.0) + p_pos)& 
             + aphiv_neg*(rho_neg*(e_neg + 0.5*(U_neg)**2.0) + p_neg) &
             + aSf*p_pos - aSf*p_neg         ! Total Energy Flux
...
!*****************************************************************************
 ...
 implicit none
 real :: uL(3), uR(3)!  Input (conservative variables rho*[1, v, E])
 real :: KNPT(3)      ! Output (numerical flux across L and R states)
!Local constants
 real :: gamma                        ! Ratio of specific heat.
 real :: zero, quarter, half, one, two
!Local variables
...
!  Left state
    ...
    U_neg    = rhoU_neg/rho_neg
    e_neg    = (ET_neg/rho_neg) - half*U_neg**2.0
    p_neg    = (gamma-1)*(ET_neg-half*rho_neg*rhoU_neg)
    cSf_neg  = gamma*p_neg/rho_neg
...
!  Right state
    ...
    U_pos    = rhoU_pos/rho_pos
    e_pos    = (ET_pos/rho_pos) - half*U_pos**2.0
      p_pos  = (gamma-1)*(ET_pos-half*rho_pos*rhoU_pos)
    cSf_pos  = gamma*p_pos/rho_pos
...
! Continuity Flux
  ...
  KNPT(1)   =  aphiv_pos*rho_pos + aphiv_neg*rho_neg   ! Coninuity Flux

  KNPT(2)   = (aphiv_pos*rhoU_pos + aphiv_neg*rhoU_neg) &
            + (a_pos*p_pos + a_neg*p_neg) ! Linear Momentum Flux

  KNPT(3) = aphiv_pos*(rho_pos*(e_pos + 0.5*(U_pos)**2.0) + p_pos)& 
             + aphiv_neg*(rho_neg*(e_neg + 0.5*(U_neg)**2.0) + p_neg) &
             + aSf*p_pos - aSf*p_neg         ! Total Energy Flux
...
end module Convective_Schemes_1D